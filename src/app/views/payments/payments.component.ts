...
export class PaymentsComponent implements OnInit {
  ...
  paymentAmountColor( transaction: any = null ) {
    if (transaction.status) {
      switch (transaction.status.toLowerCase()) {
        case 'processed':
          return 'paymentAmountColorPaid';
        case 'pending':
          return 'paymentAmountColorDefault'
        case 'cancelled':
        case 'voided':
        case 'void pending':
          return 'paymentAmountColorVoid'
        case 'failed':
          return 'paymentAmountColorFailed';
        case 'printed':
        case 'unpaid':
          return 'paymentAmountColorUnpaid';
      }
    }
    return 'paymentAmountColorDefault';
  }

  getFromName( transaction: any = null ) {
    let strName = <string> '';
    if ( transaction.u_token == this.internalDwollaId || transaction.u_token == transaction.sndr_token ) {
      strName = this.getNameFromSender( transaction );
    } else if ( transaction.u_token == transaction.rec_token ) {
      strName = this.getNameFromRecipient( transaction );
    }
    return strName;
  }

  getNameFromSender( transaction: any = null ) {
    let strName = transaction.sndr_name + '<br>';
    if ( this.canSenderEmailLink( transaction ) ) {
      strName += '<a href="/users/' + transaction.sender.dwl_token + '/' + transaction.sender.u_token + '">' + transaction.sndr_email + '</a>';
    } else {
      strName += '<span>' + transaction.sndr_email + '</span>';
    }

    return strName;
  }

  getNameFromRecipient( transaction: any = null ) {
    let strName = transaction.rec_name + '<br>';
    if ( this.canRecipientEmailLink( transaction ) ) {
      strName += '<a href="/users/' + transaction.recipient.dwl_token + '/' + transaction.recipient.u_token + '">' + transaction.rec_email + '</a>';
    } else {
      strName += '<span>' + transaction.rec_email + '</span>';
    }
